...
def fail(name, detail=""):
    results.append(Result(name, FAIL, detail))
...
# 4c. BenchmarkResult
def _s_benchmark(s):
    from apex.backend.models.decision_models import BenchmarkResult, EstimateRun
...
def _s_estimate_line(s):
    ...
    override = EstimatorOverride(
        ...
        original_value=910.0,
        overridden_value=875.0,
...
def _s_escalation(s):
    ...
    from apex.backend.models.decision_models import EscalationInput, EstimateRun
...
def _s_breakdown(s):
    from apex.backend.models.decision_models import CostBreakdownBucket, EstimateRun
    ...
    for bucket, amount in [
        ("direct_labor", 2_100_000),
        ("direct_material", 1_400_000),
        ("subcontract", 700_000),
        ("general_conditions", 650_000),
        ("contingency", 250_000),
        ("escalation", 120_000),
        ("overhead", 180_000),
        ("fee", 240_000),
...
def _s_bid_outcome(s):
    from apex.backend.models.decision_models import BidOutcome, EstimateRun

    proj = _make_project(s)
    run = EstimateRun(project_id=proj.id, version_number=1, run_status="complete", final_bid_value=5_640_000.0)
    s.add(run)
    s.flush()
    outcome = BidOutcome(
        project_id=proj.id,
...
def _s_full_chain(s):
    ...
    scope = ScopeItem(
        estimate_run_id=run.id, canonical_name="Formwork", scope_status="included_explicit", inclusion_confidence=0.9
    )
    s.add(scope)
    s.flush()

    qty = QuantityItem(
        estimate_run_id=run.id, scope_item_id=scope.id, quantity_value=5000.0, unit="SFCA", source="manual"
    )
    s.add(qty)
    s.flush()

    br = BenchmarkResult(
        estimate_run_id=run.id, scope_item_id=scope.id, sample_size=7, p50=4.20, confidence_label="low"
    )
    s.add(br)
    s.flush()
    ...
    esc = EscalationInput(estimate_run_id=run.id, category="concrete", escalation_rate=0.04, escalation_amount=12_000.0)
    s.add(esc)
    s.flush()

    bucket = CostBreakdownBucket(estimate_run_id=run.id, bucket_type="direct_labor", amount=21_000.0)
    s.add(bucket)
    s.flush()
...
def _test_benchmarking_engine():
    ...
    try:
        ...
        ok(
            f"Engine: BenchmarkingEngine (n={result.sample_size}, p50={result.p50}, confidence={result.confidence_label})"
        )
    finally:
        s.rollback()
        s.close()
...
def _test_ontology_seed():
    from apex.backend.db.ontology_seed import CANONICAL_ACTIVITIES, seed_canonical_activities
    from apex.backend.models.decision_models import ActivityAlias

    s = _Session()
    try:
        seed_canonical_activities(s)
        ...
        seed_canonical_activities(s, overwrite=False)
...
from apex.backend.services.benchmarking_engine.engine import _confidence_label, compute_confidence

score = compute_confidence(
    sample_size=11,
    unit_costs=[880, 900, 910, 920, 945],
    context_sim=0.85,
    recency=0.95,
    data_quality=0.90,
)
